...
void ClothSimulator::assignLoader(objl::Loader *load){
    this->loader= load;
}
...
//    active_shader.type_hint
  ...
  case PHONG:
    ...
    shader.setUniform("u_light_pos", Vector3f(lightP.x, lightP.y,lightP.z), false);
    ...
    // TOGGLES
      shader.setUniform("toggleSpecularShading", toggleS, false);
      shader.setUniform("toggleOutlines", toggleO, false);
      shader.setUniform("toggleDiffuseLighting", toggleL, false);
      shader.setUniform("cutOff", cutO, false);
      shader.setUniform("specularHighlight", specularH, false);
      shader.setUniform("outlineColor", oColor, false);
      shader.setUniform("outlineSize", oSize, false);
          shader.setUniform("diffuseLevel", diffuseL, false);
    drawPhong(shader);
    break;
  }
...
void ClothSimulator::drawPhong(GLShader &shader) {
    if (loader) {
        ...
        for (int i = 0; i < num_tris; i += 3) {
            ...
            objl::Vector3 p1 = vertices[indices[i]].Position;
            objl::Vector3 p2 = vertices[indices[i + 1]].Position;
            objl::Vector3 p3 = vertices[indices[i + 2]].Position;
            objl::Vector3 n1 = vertices[indices[i]].Normal;
            objl::Vector3 n2 = vertices[indices[i + 1]].Normal;
            objl::Vector3 n3 = vertices[indices[i + 2]].Normal;

            objl::Vector2 uv1 = vertices[indices[i]].TextureCoordinate;
            objl::Vector2 uv2 = vertices[indices[i + 1]].TextureCoordinate;
            objl::Vector2 uv3 = vertices[indices[i + 2]].TextureCoordinate;
            ...
            uvs.col(i * 3    ) << uv1.X, uv1.Y;
            uvs.col(i * 3 + 1) << uv2.X, uv2.Y;
            uvs.col(i * 3 + 2) << uv3.X, uv3.Y;
        ...
        shader.drawArray(GL_TRIANGLES, 0, num_tris * 2);
    } else{
        int num_tris = cloth->clothMesh->triangles.size();
...
void ClothSimulator::initGUI(Screen *screen) {
  ...
  window = new Window(screen, "Toon Modifier");
...

  {
    ...
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) { toggleS = state; });

    b = new Button(window, "Toggle Outlines");
    b->setFlags(Button::ToggleButton);
    b->setPushed(toggleO);
    b->setFontSize(14);
    b->setChangeCallback(
        [this](bool state) { toggleO = state; });

      b = new Button(window, "Toggle Diffuse Lighting");
      b->setFlags(Button::ToggleButton);
      b->setPushed(toggleL);
      b->setFontSize(14);
      b->setChangeCallback(
              [this](bool state) { toggleL = state; });
  }

    new Label(window, "Outline Color", "sans-bold");

    {
        ColorWheel *cw = new ColorWheel(window, oColor);
        cw->setColor(this->oColor);
        cw->setCallback(
                [this](const nanogui::Color &oColor) { this->oColor = oColor; });
    }
    ...
    new Label(window, "Shading Brightness", "sans-bold");

    {
        ...
        slider->setValue(specularH);
        slider->setFixedWidth(105);

        TextBox *percentage = new TextBox(panel);
        percentage->setFixedWidth(75);
        percentage->setValue(to_string(specularH));
        percentage->setUnits("%");
        ...
        slider->setFinalCallback([&](float value) {
            specularH = (double)value;
            // cout << "Final slider value: " << (int)(value * 100) << endl;
        });
    }

    new Label(window, "Shading Size", "sans-bold");

    {
        ...
        Slider *slider = new Slider(panel);
        slider->setValue(cutO);
        slider->setFixedWidth(105);

        TextBox *percentage = new TextBox(panel);
        percentage->setFixedWidth(75);
        percentage->setValue(to_string(cutO));
        percentage->setUnits("%");
        percentage->setFontSize(14);
    ...
    new Label(window, "Outline Size", "sans-bold");
  {
    Widget *panel = new Widget(window);
    panel->setLayout(
        new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 5));
    ...
    Slider *slider = new Slider(panel);
    slider->setValue(oSize);
    slider->setFixedWidth(105);

    TextBox *percentage = new TextBox(panel);
    percentage->setFixedWidth(75);
    percentage->setValue(to_string(oSize));
    percentage->setUnits("%");
    percentage->setFontSize(14);
...

  {
    ...
    fb->setFontSize(14);
    fb->setValue(lightP.x);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { lightP.x = value; });
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(lightP.y);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { lightP.y = value; });
    ...
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(lightP.z);
    fb->setUnits("m/s^2");
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { lightP.z = value; });
  }

    new Label(window, "Diffuse Lighting", "sans-bold");

    {
        ...
        new Label(panel, "Diffuse Levels :", "sans-bold");

        IntBox<int> *fsec = new IntBox<int>(panel);
        fsec->setEditable(true);
//        fsec->setFixedSize(Vector2i(100, 0));
        fsec->setFontSize(14);
        fsec->setValue(diffuseL);
        fsec->setSpinnable(true);
        fsec->setMinValue(1);
        fsec->setCallback([this](int value) { diffuseL = value; });
    }
