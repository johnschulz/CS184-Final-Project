...
int main(int argc, char **argv) {
  ...
  bool file_specified = false;
    std::string obj_to_load_from;
    bool obj_specified = false;
  
  while ((c = getopt (argc, argv, "f:w:r:a:o:")) != -1) {
    switch (c) {
      case 'f': {
  ...
  app->loadCollisionObjects(&objects);
    ...
    objl::Loader loader;
    if(obj_specified) {
        loader.LoadFile(obj_to_load_from);
        app->assignLoader(&loader);

    } else{
        app->assignLoader(nullptr);
    }
  app->init();
